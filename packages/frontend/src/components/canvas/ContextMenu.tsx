...
import { Plus, Copy, Trash2, Group, Ungroup, Clipboard, Pencil } from 'lucide-react';
...
export function ContextMenu({ state, onClose, onAddScene, onDeleteNode, onEditNode }: Props) {
  ...
  const items = isNode
    ? [
        ...(editableTypes.includes(state.nodeType || '') ? [
          { icon: Pencil, label: 'Editar', shortcut: '', action: () => onEditNode?.(state.nodeId!, state.nodeType!, state.nodeData), danger: false },
          { type: 'separator' as const },
        ] : []),
        { icon: Copy, label: 'Duplicar', shortcut: '⌘D', action: onClose },
        { type: 'separator' as const },
        { icon: Trash2, label: 'Eliminar', shortcut: '⌫', action: () => { onDeleteNode?.(state.nodeId!); onClose(); }, danger: true },
      ]
    : [
        { icon: Plus, label: 'Añadir escena aquí', shortcut: '', action: () => onAddScene(state.x, state.y) },
        { type: 'separator' as const },
        { icon: Copy, label: 'Pegar', shortcut: '⌘V', action: onClose },
        { type: 'separator' as const },
        { icon: Trash2, label: 'Eliminar selección', shortcut: '⌫', action: onClose, danger: true },
      ];
